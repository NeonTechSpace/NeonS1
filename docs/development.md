...
| People and world | Partial | People, relationships, schedules, map projection, shops, properties, services, access zones, employee navigation data, and normalized static vehicle graph and endpoint evidence | Vehicle graph layers remain separate, endpoint offsets are not traversable evidence, route composition is not implemented, and there is no live map or game connection |
