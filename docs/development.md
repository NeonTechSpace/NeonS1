...
| People and world | Partial | People, relationships, schedules, map projection, shops, properties, services, access zones, employee navigation data, normalized static vehicle graph and endpoint evidence, and independent per-layer vehicle route candidates | Vehicle route candidates use geometric graph-edge distance only. Layer composition, endpoint-offset traversal, native path choice, and live driving remain unsupported |
| Property blueprints | Partial | Placement validation, construction order, item cost, normalized storage slot summaries, collision, access, temperature, production capacity, exact installed-pot sprinkler coverage and cycle timing, schedules, transfers, routing inputs, logistics, assigned employee service time, work priority, movement rules, endpoint reachability, deterministic empty-destination movement allocation with compatible-slot reservations, selected navigation-graph routes and source-to-destination traversal time, selected endpoint and route physical feasibility against normalized employee dimensions and static geometry, aggregate construction cost, configured daily wages, installed capacity, selected schedule duration, evidence-gated realized profit, operational blockers, and Cleaner bin assignment limits and eligibility | Does not evaluate current storage contents or available slots, simulate mutable pot moisture or live trash state, prove exact clearance for unsupported collider shapes or orientations or colliders without activity evidence, prove Cleaner endpoint reachability or runtime collection completion, establish exact employee travel from current position with return legs and runtime task order, model probabilistic expected profit, or provide a blueprint editor or public browser |
...
Exporter version `0.0.30` targets *Schedule I* `0.4.6f13`, MelonLoader `0.7.3`, S1API `3.1.6`, and AssetRipper `1.3.14`.
A newer game or dependency version requires a new build and extraction audit.

Normalizer version `0.0.39` defines the current normalized output contract.
