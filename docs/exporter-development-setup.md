...
NeonS1 includes an in-game data exporter and an offline asset extractor.
...
2. Set `NEONS1_GAME_DIR` to the Schedule I directory or pass the `GameDirectory` MSBuild property
3. Build `projects/game-data-exporter/in-game-exporter/NeonS1.GameDataExporter.csproj` in Release configuration
4. Copy `NeonS1.GameDataExporter.dll` into the game's `Mods` directory
...
8. Publish `projects/game-data-exporter/offline-asset-extractor/NeonS1.OfflineAssetExtractor.csproj` as a self-contained Windows x64 executable
