...
Version `0.0.1` was built against Schedule I `0.4.6f11`, MelonLoader `0.7.3`, S1API `3.1.6`, and AssetRipper `1.3.14`.
...
Provide the installation path through `NEONS1_GAME_DIR` or the `GameDirectory` MSBuild property.

PowerShell:

```powershell
$env:NEONS1_GAME_DIR = 'C:\Program Files (x86)\Steam\steamapps\common\Schedule I'
dotnet build '.\in-game-exporter\NeonS1.GameDataExporter.csproj' -c Release
```

Bash on Windows:

```bash
export NEONS1_GAME_DIR='C:/Program Files (x86)/Steam/steamapps/common/Schedule I'
dotnet build './in-game-exporter/NeonS1.GameDataExporter.csproj' -c Release
```
...
```powershell
dotnet build '.\in-game-exporter\NeonS1.GameDataExporter.csproj' -c Release -p:GameDirectory='C:\Program Files (x86)\Steam\steamapps\common\Schedule I'
```
...
```text
in-game-exporter/bin/Release/net6.0/NeonS1.GameDataExporter.dll
```
...
1. Close Schedule I.
2. Remove or disable another copy of `NeonS1.GameDataExporter.dll` so only one exporter handles the save-load event.
3. Copy `NeonS1.GameDataExporter.dll` into the game's `Mods` directory.
...
```text
<Schedule I>/UserData/NeonS1/exports
```

A normal Steam launch is expected to use the Schedule I directory as that working directory.

Set `NEONS1_EXPORT_OUTPUT` before starting the game to use another directory.
Steam must inherit that environment variable, so restart Steam after changing a persistent user environment variable.

Each run creates:

```text
neons1-game-data-<UTC run id>.json
neons1-game-data-<UTC run id>.json.sha256
neons1-assets-<UTC run id>/
```

The report schema is `neons1-game-data-export-1`.
...
```powershell
dotnet publish '.\offline-asset-extractor\NeonS1.OfflineAssetExtractor.csproj' -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false
```
...
```text
offline-asset-extractor/bin/Release/net10.0/win-x64/publish/NeonS1.OfflineAssetExtractor.exe
```
...
```powershell
& '.\offline-asset-extractor\bin\Release\net10.0\win-x64\publish\NeonS1.OfflineAssetExtractor.exe' `
  --report 'D:\NeonS1\exports\neons1-game-data-<run id>.json' `
  ...
  --output 'D:\NeonS1\offline-assets\<run id>' `
  --checkpoint 5
```
...
```bash
'./offline-asset-extractor/bin/Release/net10.0/win-x64/publish/NeonS1.OfflineAssetExtractor.exe' \
  --report 'D:/NeonS1/exports/neons1-game-data-<run id>.json' \
  --assetripper 'D:/Tools/AssetRipper-1.3.14/AssetRipper.GUI.Free.exe' \
  --game-data 'C:/Program Files (x86)/Steam/steamapps/common/Schedule I/Schedule I_Data' \
  --output 'D:/NeonS1/offline-assets/<run id>' \
  --checkpoint 5
...
Its manifest schema is `neons1-offline-mesh-export-1`.
It writes:
