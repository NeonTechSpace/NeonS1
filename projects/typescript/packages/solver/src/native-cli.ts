...
async function resolveExportDirectory(options: CliOptions): Promise<string> {
    ...
    const configuredGameDirectory = options.gameDirectory ?? process.env.NEONS1_GAME_DIR;
    if (configuredGameDirectory === undefined || configuredGameDirectory.trim() === '') {
        throw new Error('Pass --game-directory or --export-directory, or set NEONS1_GAME_DIR');
    }
    ...
    return path.join(gameDirectory, 'UserData', 'NeonS1', 'exports');
}
...
Options:
  ...
  --game-directory PATH    Schedule I directory; defaults to NEONS1_GAME_DIR
