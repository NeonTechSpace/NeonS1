...
interface ProductCheckpoint {
    readonly schema: 'neons1-recipe-corpus-product-checkpoint-1';
...
interface RunLock {
    readonly schema: 'neons1-recipe-corpus-run-lock-1';
...
async function generateProduct(
    ...
    const checkpoint: ProductCheckpoint = {
        schema: 'neons1-recipe-corpus-product-checkpoint-1',
        coverageKey,
        productId,
        files,
    };
...
function parseCheckpoint(value: unknown): ProductCheckpoint {
    ...
    if (record.schema !== 'neons1-recipe-corpus-product-checkpoint-1' ||
...
async function acquireLock(lockPath: string): Promise<() => Promise<void>> {
    const lock: RunLock = {
        schema: 'neons1-recipe-corpus-run-lock-1',
        token: randomUUID(),
        pid: process.pid,
...
async function readLock(lockPath: string): Promise<RunLock | null> {
    try {
        ...
        return value.schema === 'neons1-recipe-corpus-run-lock-1' &&
